...
- **Cancel/renew clocks (2026-08-19):** mig `0020` — `cancelled_at` (reject/owner cancel once) + `renewed_at` (last renew). Cancelled paints `cancelledAt` not later `updatedAt`. Renewed meta under Due, not as Approved. Profile RQ keeps `approvedAt`. Waive stays on `updatedAt`.
