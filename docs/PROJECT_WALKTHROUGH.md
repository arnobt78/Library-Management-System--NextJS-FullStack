...
- Insights polish (2026-08-17): 8 KPI / 8 chart ops dashboard; overdue DataTable + ids; shared opsPeriod; cross-domain chips; still no invent densify for chart series. Overdue period = daysOverdue severity (buckets/floors). Follow-up: Title Case / DNA table / filter chips / delta KPI badges; `TicketDateMeta` Updated —; `seed:demo` vs `seed:reset`. Header All History + LabelList + overdue Actions kebab.
