...
- **Fines Platform closeout (2026-08-18):** Single JS engine `lib/fines` (`computeLiveFineForRow`, `mapDisplayFine`, `fine_rate_history` pro-rata). Live display on profile, borrow queue/detail, Insights overdue table + KPI, User 360 history + outstanding KPI, export, reminders. Admin: waive/adjust/paid/stamp, fine-free return, dispute ticket deep-link, Force Update densify. Cron: `due-reminders` 07:00, `stamp-overdue-fines` 08:00 (sync ACCRUING → stamp). `fine.write` → `commitMutationCache` + RSC invalidate (no chart invent-densify). Smoke: dispute dialog, force-update detail fine, rate-change profile KPI.
