...
> Parent: REQ-0018, REQ-0024, CR-0002, CR-0003 | Updated: 2026-08-19 | Status: C2 Stage 4; admin fine UI + book links; Gate 2 blocked (EvalGate nonlocal)
...
- Activity History: admin-visible lifecycle audit (borrow create, reservation/renewal, registration re-apply, Automation exports) + Entity routes (ops/export/recs → `/admin/automation`); PrefetchLink staleTime 0; recs densify blocks SSR featured reseed. Hard-delete unlink: `markActivityEntityDeleted` + SSR `annotateMissingActivityEntities` (book/review/ticket/user/borrow); soft reject/cancel stay linkable. Detail delete never-paint 404: missing book/ticket/review (admin + public ticket) → `redirect` list + settle soft-nav; branded `not-found` fallback only.
- Admin people: Registration Queue · Admin Requests · User Directory; **unified User 360** (`AdminUser360Shell`, entries directory|registration|privilege); privilege/reservations/activity RQ densify; Insights SSR-only; `prefetchAdminUser360Caches`; PrefetchLink UUID warm (`staleTime: 0`). Fine KPI + open-overdue Fine cells = live days × rate.
- User 360 tables: `USER_360_TABLE` fixed; Borrowing 44/34/10/12; Reviews 44/12/44 title→`/admin/books/[id]` + `AdminBookContextLinks` (View book/borrow/review detail); Status PENDING Submitted / decided `DecisionActorStack`; Reservations badge→Requested; `AdminBookIdentityCell` admin-first default.
...
- Insights polish (2026-08-17): 8 KPI / 8 chart ops dashboard; overdue DataTable + ids; shared opsPeriod; cross-domain chips; still no invent densify for chart series. Overdue period = daysOverdue severity (buckets/floors). Follow-up: Title Case / DNA table / filter chips / delta KPI badges; `TicketDateMeta` Updated —; `seed:demo` vs `seed:reset`. Header All History + LabelList + overdue Actions kebab. C2-v2 outstanding = live days × daily rate (matches overdue table).
...
- Book trailer (2026-08-16/17): `BookVideo` max-h + `object-contain`; **optional** trailer (mig `0016` nullable `video_url`, Zod empty OK, form no `*`, assert only when URL set; empty → placeholder / admin —).
...
- **Fines Platform closeout (2026-08-18):** Single JS engine `lib/fines` (`computeLiveFineForRow`, `mapDisplayFine`, `fine_rate_history` pro-rata). Live display on profile, borrow queue/detail, Insights overdue table + KPI, User 360 history + outstanding KPI, export, reminders. Admin: waive/adjust/paid/stamp, fine-free return, Force Update densify. Cron: `due-reminders` 07:00 (sync ACCRUING + reminders), `stamp-overdue-fines` 08:00 (sync + stamp). `fine.write` → `commitMutationCache` + RSC invalidate.
- **Fines polish (2026-08-18):** Admin borrow detail inline dispute dialog (`SupportTicketDialog` light) files ticket on behalf of borrower via `requesterUserId`; user `/support-tickets?create=1` deep-link unchanged. Dead `liveFineAmountSql` removed; export keeps flat `liveOutstandingFineSumSql` only.
- **Fines display (2026-08-19):** UTC calendar overdue (due today is not overdue). `resolveReturnFine` keeps WAIVED `$0` / PAID stored on user Return. Automation Force Update skips WAIVED/PAID (`isImmutableFineStatus`). Profile chips + Due in 48h titles. Open STAMPED still live days × rate. Insights charts: evict+refetch only (DEC-0114). User 360 Fine/Overdue KPIs + advisory Next Actions: `users.fineMetrics` + `useUserFineMetrics` densify on `fine.write` / `borrow.lifecycle` (DEC-0118). **Admin fine UI + links (2026-08-19):** `lib/admin/adminRoutes` + `AdminBookContextLinks`; admin tables title→`/admin/books/[id]`; borrow detail Fine Balance vertical stack + `LIGHT_MENU` Fine Actions; KPI refetch guards (`isPartialBorrowCacheForUser`, `dailyRate=0` skip).
- **Due/return clocks (2026-08-19):** mig `0019` — `due_date`/`return_date` timestamptz (due = UTC noon of calendar day; return = `now()`); `approved_at` set only on approve. Due paints date-only; Approved/Returned use local clocks. Queue Fine Waived; waived Fine KPI is not Accrued.
- **Cancel/renew clocks (2026-08-19):** mig `0020` — `cancelled_at` (reject/owner cancel once) + `renewed_at` (last renew). Admin queue/detail prod-smoked: Due date-only; Fine Waived; Approved ≠ waive/renew. Profile cards paint Cancelled + `renewed` under Due after the follow-up tip. Waive stays on `updatedAt`.
...
- Sign-in demo dropdown: `TEST_ACCOUNTS` (Test User / Test Admin). `npm run seed:reset` = 17 books + 2 accounts; `npm run seed:demo` = rich ops overlay (borrows/fines/holds/reviews/tickets/notifications/activity, both accounts, staggered dates). Portable reuse: `docs/PORTABLE_AUTH_UI_GUIDE.md`.
...
- Runtime: `package.json` engines `node` `24.x` (Vercel). Sentry: `@sentry/nextjs` + `/api/monitoring` tunnel; `withSentryConfig` quiet unless `SENTRY_VERBOSE=1`, `telemetry: false` (sourcemaps still upload); DSN/org/project/token via env (see `.env.example`). Keep immutable `/_next/static` Cache-Control (Next may warn in dev). Book create: Added-only stamps; Activity uses action actor; media assert 10s+retry; CopyableText break-all. Delete densify: all-books total+page backfill. Genres RQ densify + delete KPI snapshot fallback.
