...
- Densify instant UI closeout (2026-08-19): SSR `initialDataUpdatedAt` on All Users/ticket/review/Fine/Automation hooks; `BookCollection` uses `useUserBorrows`; `users.fineMetrics` + `useUserFineMetrics` densify User 360 Fine/Overdue KPI + Next Actions on `fine.write`/`borrow.lifecycle`; Insights PrefetchLink warm; charts evict+refetch (DEC-0118). Prove 353 unit + build PASS. Prod smoke PASS: waive Assembly $38 → User 360 Fine $55→$17 + Next Actions; queue Waived; Insights $17.
