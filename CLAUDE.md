...
- Demo seed: `npm run seed:reset` (`scripts/reset-and-seed.ts`) wipes FK-safe transactional tables, reseeds `TEST_ACCOUNTS` then 17 `dummybooks.json` books (`availableCopies` = total; `created_by` = test@admin.com, `updated_by` null → Updated —) + `status_reviewed_*` stamps; queues/reviews/tickets/holds/activity intentionally empty for one-by-one testing. Mixed ops: `npm run seed:demo` (`scripts/seed-demo-ops.ts`) clears overlays only and inserts borrows/holds/tickets/reviews/activity.
...
- Fines polish (2026-08-18): dead `liveFineAmountSql` removed; due-reminders 07:00 sync ACCRUING + `fine.write`; admin borrow-detail inline dispute dialog + on-behalf `requesterUserId` (Zod/API); stamp skips WAIVED/PAID; integration 12/12 + 327 unit + build PASS.
