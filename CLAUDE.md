...
- After audit/prove: compact-update `CLAUDE.md` + `docs/PROJECT_WALKTHROUGH.md` + living `.agile-v/` (STATE, CHANGE_LOG, DECISION_LOG, TRACE_LOG, BACKLOG). No `.claude/` dir in this repo.
- UI smoke on prod `https://university-library-managment.vercel.app/` (demo `test@admin.com` / `test@user.com`), not localhost, unless the owner names a local-only check.
...
- Migration `0016_books_video_url_nullable.sql`: `books.video_url` nullable (optional trailer). Down: `0016_books_video_url_nullable.down.sql`. Apply before create-without-trailer.
- Migration `0018_fine_rate_history.sql` applied; down `0018_fine_rate_history.down.sql`.
- Migration `0019_borrow_due_return_timestamptz.sql` applied on shared DB (2026-08-19): `due_date`/`return_date` timestamptz (UTC noon due) + `approved_at`. Down: `0019_borrow_due_return_timestamptz.down.sql`. Apply on other envs before matching seed/code.
- Migration `0020_borrow_cancelled_renewed_at.sql` applied on shared DB (2026-08-19): `cancelled_at` (reject/owner cancel) + `renewed_at` (last renew). Down: `0020_borrow_cancelled_renewed_at.down.sql`. Apply on other envs before matching seed/code.
...
- Demo seed: `npm run seed:reset` then `npm run seed:demo` — reset = 17 books + 2 accounts; demo = rich ops overlay (borrows all statuses/fine_status, holds WAITING/READY/EXPIRED/FULFILLED, reviews PENDING/APPROVED/REJECTED, tickets all statuses + replies, notifications, activity, fine_rate_history, admin request, both test accounts).
...
- Book trailer (2026-08-16/17): detail `BookVideo` frame `max-h-[min(50vh,28rem)]` + `object-contain`; trailer optional (Zod + mig `0016` nullable `video_url`; form no `*`; empty → placeholder / admin —).
...
- Fines Platform closeout (2026-08-18): `lib/fines/mapDisplayFine` pro-rata on list/detail/profile/Insights/User360/export/reminders; `fineStatus` lifecycle; densify via `patchBorrowFineUpdate`/`densifyOverdueFines`; fine-free return + dispute deep-link; `AdminAdjustFineDialog`; ACCRUING sync in stamp cron; mig `0017`/`0018`; Prove 323 tests + build PASS.
- Fines polish (2026-08-18): dead `liveFineAmountSql` removed; due-reminders 07:00 sync ACCRUING + `fine.write`; admin borrow-detail inline dispute dialog + on-behalf `requesterUserId` (Zod/API); stamp skips WAIVED/PAID; Borrow Queue `parseStoredFine`; integration 12/12 + 327 unit + build PASS.
- Fines display (2026-08-19): calendar overdue (profile/timer/Insights `(due_date AT TIME ZONE 'UTC')::date < CURRENT_DATE`); `resolveReturnFine` keeps WAIVED `$0` / PAID stored; return densify `fineStatus`+`displayFineAmount`; Force Update skips WAIVED/PAID (`isImmutableFineStatus`); profile Waived/Paid chips + Due in 48h titles. Prove 335 unit. No invent Insights/User 360 Fine KPI densify.
- Due/return clocks (2026-08-19): mig `0019` timestamptz due (UTC noon) + return `now()` + `approved_at` (approve only). Due paints date-only; Approved/Returned local clocks. Queue Fine **Waived** not —. Waived Fine KPI not “Accrued”. Prove 347 unit.
- Cancel/renew clocks (2026-08-19): mig `0020` `cancelled_at` + `renewed_at`. Reject/owner cancel stamp cancelled; renew stamps last renew (not Approved). Profile RQ map keeps `approvedAt`. Prod smoke 2026-08-19 PASS on queue/detail (Due date-only; Software Fine **Waived**; Distilled Approved ≠ Renewed; Fundamentals Renewed ≠ Approved; Lean Paid $5). Profile history now paints Cancelled/`renewed` clocks (deploy after this tip). Prove 350 unit.
- Densify instant UI closeout (2026-08-19): SSR `initialDataUpdatedAt` on All Users/ticket/review/Fine/Automation hooks; `BookCollection` uses `useUserBorrows`; `users.fineMetrics` + `useUserFineMetrics` densify User 360 Fine/Overdue KPI + Next Actions on `fine.write`/`borrow.lifecycle`; Insights PrefetchLink warm; charts evict+refetch (DEC-0118). Fine KPI delta densify (`patchUserFineMetricsDelta`) + partial refetch guard (`isPartialBorrowCacheForUser`, dailyRate=0 skip) fixes back-nav $0 after waive. Admin book links: `lib/admin/adminRoutes` + `AdminBookContextLinks`; `AdminBookIdentityCell` default `/admin/books/[id]`; borrow/review secondary links on queue/360/Insights. Borrow detail Fine Balance KPI stack + `LIGHT_MENU` Fine Actions. Prove 358 unit + build PASS.
- Demo seed (2026-08-18): enriched `seed:demo` — full ops overlay (borrows/fines/holds/reviews/tickets/notifications/activity, both test accounts).
- Insights polish (2026-08-17): 8 KPI StatCards + badges; overdue DataTable (`InsightsOverdueTable`); shared `opsPeriod` → trends/monthly/overdue analysis; 8 charts (fine forecast, genre pressure, monthly); cross-domain PrefetchLink chips; SSR snapshot + staleTime 0 / no invent densify. Overdue filters use `daysOverdue` severity buckets/floors (not dueDate windows). Insights DNA + Title Case + delta-only KPI badges; ticket Updated shows — until real edit (`TicketDateMeta`); `seed:demo` mixed ops. Header period (default All History) + chart LabelList; overdue Actions kebab. C2-v2 outstanding = live days × daily rate (same as overdue table; not stored `fine_amount`).
...
- User 360 status KPIs: `AdminUser360StatusKpiRow` densifies Reg/Privilege via `signupRequestDetail`/`users.detail` (header/panel keys); Fine/Overdue SSR (live days × rate; no invent densify).
...
- Book densify+createdBy (2026-08-14/15): `loadBookWithUpdater` SSR/API; mig `0015`; seed + create stamp `createdBy` only (`updatedBy` null → Updated —); Activity densify uses action actor (not session-only / All admin); media assert 10s + one abort retry; `CopyableText` `break-all`.
