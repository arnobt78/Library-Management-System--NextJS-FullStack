...
- Demo accounts: `TEST_ACCOUNTS` + `npm run seed:test-profiles`; avatars `/images/profile-img*.png` via `UserAvatar`/`resolveUniversityCard`.
- Auth ops: apply `0009_users_audit_fields.sql` before schema selects `users.updated_at`/`updated_by`; rehash-on-login is non-fatal. Legacy + scrypt both verify; keep prod deploy in sync with password format in shared DB.
