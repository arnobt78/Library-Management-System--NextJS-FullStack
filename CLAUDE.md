...
- Agile V: C2 active; Gate 1 `GATE-0006` + CR-0003 `GATE-0007`; tip `b9c5e4d` / HEAD `bee0eef` (people IA + cross-domain densify); Wave 5/EvalGate FAIL still blocks C2 Gate 2 (nonlocal evidence).
...
- Admin people IA: Registration Queue + Admin Requests + User Directory (separate nav); TanStack queues + detail routes `/admin/account-requests/[userId]`, `/admin/admin-requests/[id]`; Users kebab directory; User 360 tickets/reviews/privilege/activity + header actions; densify `user.write`/`admin-request.write` RSC paths include new routes; signup detail RQ (`signupRequestDetail`) + `admin.requestDetailRoot` invalidate; PrefetchLink UUID warm detail.
