...
- Agile V: C2 active; Gate 1 `GATE-0006` + CR-0003 `GATE-0007`; tip `3e6f597` / HEAD `6d7e7f6`; Wave A done (local); Wave 5/EvalGate FAIL blocks Gate 2.
...
- Book form Wave A (2026-08-15): shell/CTA/labels; media trio dropzone+`fill` ColorPicker+Clear; `showToast.file` folder titles (no double ✅); ImageKit purge after book update/delete + `user:delete` card (`imagekitPurge`/`scheduleImageKitPurge`).
