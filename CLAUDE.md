...
- Cancel/renew clocks (2026-08-19): mig `0020` `cancelled_at` + `renewed_at`. Reject/owner cancel stamp cancelled; renew stamps last renew (not Approved). Profile RQ map keeps `approvedAt`. Prove 350 unit. Owner prod smoke: Distilled approve ≠ waive, Cancelled/renew clocks, Due date-only, Queue Fine Waived.
