...
- Agile V: C2 active; Gate 1 `GATE-0006` + CR-0003 `GATE-0007`; tip `ad1cc69` / HEAD `e2fbd4b`; ImageKit upload-limit deferred; EvalGate FAIL blocks Gate 2.
...
- Fines display (2026-08-19): calendar overdue (profile/timer/Insights `due_date < CURRENT_DATE`); `resolveReturnFine` keeps WAIVED `$0` / PAID stored; return densify `fineStatus`+`displayFineAmount`; Force Update skips WAIVED/PAID (`isImmutableFineStatus`); profile Waived/Paid chips + Due in 48h titles. Prove 335 unit. No invent Insights/User 360 Fine KPI densify.
